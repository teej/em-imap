      ...
      def listen_for_tagged_response(command)
        command.listen do |response|
          if response.is_a?(Net::IMAP::TaggedResponse) && response.tag == command.tag
            case response.name
            when "NO"
              command.fail response #Net::IMAP::NoResponseError.new((RUBY_VERSION[0,3] == "1.8" ? response.data.text : response))
            when "BAD"
              command.fail response #Net::IMAP::BadResponseError.new((RUBY_VERSION[0,3] == "1.8" ? response.data.text : response))
            else
